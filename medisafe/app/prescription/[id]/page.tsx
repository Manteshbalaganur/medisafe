  ...
  if (!prescription) {
    return (
      <div className="container mx-auto p-8 mt-20 text-center">
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Prescription not found</h2>
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 mt-16 max-w-6xl space-y-8 pb-24">
      <Button 
        variant="ghost" 
        onClick={() => router.push('/')} 
        className="group text-slate-500 hover:text-slate-900 dark:hover:text-white -ml-2"
      >
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <Badge className={
                    prescription.overallScore === 'Safe' ? 'bg-emerald-50 text-emerald-700 border-emerald-100 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-800' :
                    prescription.overallScore === 'Caution' ? 'bg-amber-50 text-amber-700 border-amber-100 dark:bg-amber-900/20 dark:text-amber-400 dark:border-amber-800' :
              ...
              {prescription.interactions.length > 0 ? (
                prescription.interactions.map((interaction, i) => (
                  <div key={i} className="space-y-2">
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className="border-rose-200 text-rose-700 bg-white dark:bg-slate-900">
                ...
                <p className="text-xs font-semibold text-slate-500">No interactions detected.</p>
              )}
            </CardContent>
          </Card>
              ...
              <AnimatePresence>
                {mealResult && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="pt-2 space-y-2">
                    <div className={`p-3 rounded-xl border flex items-start gap-2 ${mealResult.safe_to_eat ? 'bg-emerald-50 border-emerald-100 text-emerald-800 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-800' : 'bg-rose-50 border-rose-100 text-rose-800 dark:bg-rose-900/20 dark:text-rose-400 dark:border-rose-800'}`}>
          ...
          <Card className="border-slate-200 dark:border-slate-800 shadow-md bg-white dark:bg-slate-900 overflow-hidden">
            ...
            <CardContent className="space-y-4">
              ...
              <AnimatePresence>
                {pillResult && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="pt-2 space-y-2">
                    <div className={`p-3 rounded-xl border flex items-start gap-2 ${pillResult.match ? 'bg-emerald-50 border-emerald-100 text-emerald-800 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-800' : 'bg-rose-50 border-rose-100 text-rose-800 dark:bg-rose-900/20 dark:text-rose-400 dark:border-rose-800'}`}>
